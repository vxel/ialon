 ...
 * <p>
 * Like {@link Pole}, the geometry only depends on the (final) direction, width and height, so the rotated
 * normals, the 8 corner vertices and the UV coordinates are precomputed once in the constructor and emitted
 * through the shared {@link Shape#emitQuad} helper.
 *
...
public class ShortPole implements Shape {
    ...
    // Corner vertices : b* on the bottom plane (y=-0.5), t* on the top plane (y=height), named by the sign
    ...
    public ShortPole(Direction direction, float widthExtend, float height) {
        ...
        this.tMM = new Vector3f(-te, h, -te);
        this.tMP = new Vector3f(-te, h, te);
        this.tPM = new Vector3f(te, h, -te);
        this.tPP = new Vector3f(te, h, te);
