...
void main(){
   ...
   vec3 viewDir = normalize(-wvPosition);
      ...
      vPos = wvPosition;
        ...
        float lightLevel = min(float(SunIntensity) * float(SunIntensity) / 225.0, AmbientSum.r);
        lightLevel = max(lightLevel, float(TorchIntensity) * float(TorchIntensity) / 225.0);
