...
public class Cube implements Shape {
    ...
    public void add(Vec3i location, Chunk chunk, ChunkMesh chunkMesh) {
        ...
        Direction faceUp = UP;
        Direction faceDown = DOWN;
        Direction faceWest = WEST;
        Direction faceEast = EAST;
        Direction faceSouth = SOUTH;
        Direction faceNorth = NORTH;

        if (chunk.isFaceVisible(location, faceUp)) {
            boolean flip = enlightFace(location, faceUp, chunk, chunkMesh);
            createUp(location, chunkMesh, blockScale, multipleImages, flip);
        }
        if (chunk.isFaceVisible(location, faceDown)) {
            boolean flip = enlightFace(location, faceDown, chunk, chunkMesh);
            createDown(location, chunkMesh, blockScale, multipleImages, flip);
        }
        if (chunk.isFaceVisible(location, faceWest)) {
            boolean flip = enlightFace(location, faceWest, chunk, chunkMesh);
            createWest(location, chunkMesh, blockScale, multipleImages, flip);
        }
        if (chunk.isFaceVisible(location, faceEast)) {
            boolean flip = enlightFace(location, faceEast, chunk, chunkMesh);
            createEast(location, chunkMesh, blockScale, multipleImages, flip);
        }
        if (chunk.isFaceVisible(location, faceSouth)) {
            boolean flip = enlightFace(location, faceSouth, chunk, chunkMesh);
            createSouth(location, chunkMesh, blockScale, multipleImages, flip);
        }
        if (chunk.isFaceVisible(location, faceNorth)) {
            boolean flip = enlightFace(location, faceNorth, chunk, chunkMesh);
            createNorth(location, chunkMesh, blockScale, multipleImages, flip);
        }
    }

    private boolean enlightFace(Vec3i location, Direction face, Chunk chunk, ChunkMesh chunkMesh) {
        Vector4f color = chunk.getLightLevels(location, face);
        List<Vector4f> colors = chunkMesh.getColors();

        if (face == UP) {
            Block[] nb = chunk.getNeighboursY(location, 1);

            // NE
            int a00 = chunk.vertexAO(nb[0], nb[2], nb[1]);
            colors.add(chunk.applyAO(color, a00));

            // NW
            int a01 = chunk.vertexAO(nb[6], nb[0], nb[7]);
            colors.add(chunk.applyAO(color, a01));

            // SE
            int a10 = chunk.vertexAO(nb[2], nb[4], nb[3]);
            colors.add(chunk.applyAO(color, a10));

            // SW
            int a11 = chunk.vertexAO(nb[4], nb[6], nb[5]);
            ...
            int a00 = chunk.vertexAO(nb[6], nb[0], nb[7]);
            colors.add(chunk.applyAO(color, a00));

            int a01 = chunk.vertexAO(nb[0], nb[2], nb[1]);
            colors.add(chunk.applyAO(color, a01));

            int a10 = chunk.vertexAO(nb[4], nb[6], nb[5]);
            colors.add(chunk.applyAO(color, a10));

            int a11 = chunk.vertexAO(nb[2], nb[4], nb[3]);
            colors.add(chunk.applyAO(color, a11));

            return (a00 + a11 > a01 + a10);

        } else if (face == EAST) {
            Block[] nb = chunk.getNeighboursX(location, 1);
            ...
            int a00 = chunk.vertexAO(nb[4], nb[6], nb[5]);
            colors.add(chunk.applyAO(color, a00));

            int a01 = chunk.vertexAO(nb[2], nb[4], nb[3]);
            colors.add(chunk.applyAO(color, a01));

            int a10 = chunk.vertexAO(nb[6], nb[0], nb[7]);
            colors.add(chunk.applyAO(color, a10));

            int a11 = chunk.vertexAO(nb[0], nb[2], nb[1]);
            colors.add(chunk.applyAO(color, a11));

            return (a00 + a11 > a01 + a10);

        } else if (face == SOUTH) {
            Block[] nb = chunk.getNeighboursZ(location, 1);
            ...
            int a01 = chunk.vertexAO(nb[4], nb[6], nb[5]);
            colors.add(chunk.applyAO(color, a01));

            int a10 = chunk.vertexAO(nb[0], nb[2], nb[1]);
            colors.add(chunk.applyAO(color, a10));
